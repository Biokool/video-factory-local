...
def draw_partial(ctx, pts, frac, color, lw=5, dash=None, glow=False):
    ...
    sub = pts[:n]
    if glow:
        for gw in [lw*3.5, lw*2.5, lw*1.6]:
            ctx.set_source_rgba(min(1, color[0]+.25), min(1, color[1]+.25),
                                min(1, color[2]+.25), .35)
...
def _label(ctx, lx, ly, tx, ty, text, fs=15, col=(1, 1, 1), alpha=1.0):
    if alpha <= 0.02:
        return
    ...
    pad = 6
    ctx.set_source_rgba(0, 0, 0, .78 * alpha)
    _rrect(ctx, lx - pad, ly - pad, tw + 2*pad, th + 2*pad + 4, 5); ctx.fill()
    ctx.set_source_rgba(col[0], col[1], col[2], alpha)
    ctx.move_to(lx, ly + th); ctx.show_text(text)
    ctx.set_line_width(1.5); ctx.set_source_rgba(.7, .7, .85, alpha)
    ctx.set_dash([4, 4])
    ctx.move_to(lx, ly + th/2); ctx.line_to(tx, ty); ctx.stroke(); ctx.set_dash([])
    ctx.set_source_rgba(1, 1, 1, alpha)
    ctx.arc(tx, ty, 4, 0, 6.283); ctx.stroke()
...
def render_frame(scene, hand, concept_infos, ki, nk, W, H, sid, tot, cfg):
    ...
    # fade-in sutil de la mano
    hand_h = int(H * (0.66 if sid == 3 else 0.72))
    hand_im = hand.resized(hand_h)
    ...
    if sid == 1:
        ...
        for i, name in enumerate(order):
            start = 0.10 + i * 0.20
            frac = max(0.0, min(1.0, (prog - start) / 0.18))
            draw_partial(ctx, lines[name], frac, LCOL[name],
                         lw=6 if name == hl else 4, dash=DASH[name], glow=(name == hl))
        # montes con fade
        ma = max(0.0, min(1.0, (prog - 0.55) / 0.2))
        if ma > 0:
            for name, (mx, my, mr) in montes.items():
                c = MCOL[name]
                ctx.set_source_rgba(c[0]*.35, c[1]*.35, c[2]*.35, .6 * ma)
                ctx.arc(mx, my, mr + 4, 0, 6.283); ctx.fill()
                ctx.set_source_rgba(c[0], c[1], c[2], ma)
                ctx.arc(mx, my, mr * (0.6 + 0.4 * ma), 0, 6.283); ctx.fill()
                ctx.set_source_rgba(1, 1, 1, .8 * ma)
                ctx.set_line_width(2); ctx.arc(mx, my, mr, 0, 6.283); ctx.stroke()
        # labels al final
        la = max(0.0, min(1.0, (prog - 0.75) / 0.2))
        if la > 0:
            _label(ctx, box[0] + box[2] + 30, box[1] + 40,
                   *reversed_point(montes["venus"][:2]), "Monte de Venus", 15, alpha=la)
            _label(ctx, box[0] - 260, box[1] + 80,
                   *reversed_point(lines["vida"][len(lines["vida"])//2]), "Linea de la Vida",
                   15, col=LCOL["vida"], alpha=la)
            _label(ctx, box[0] - 260, box[1] + 140,
                   *reversed_point(lines["corazon"][len(lines["corazon"])//2]),
                   "Linea del Corazon", 15, col=LCOL["corazon"], alpha=la)
